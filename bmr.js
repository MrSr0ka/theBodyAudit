...
function getBmrKcal() {
  ...
  bmrTotal = bmrPattern = 66.47 + 13.75 * weight + 5.003 * height - 6.755 * age;
  ...
  document.getElementById(
    "proteinsText"
  ).innerText = `You need ${proteinsMacro.toFixed(
    0
  )} calories coming from Protiens. Proteins total
      percent is ${proteinsPercent}%. Which consists of ${proteinsGrams.toFixed(
    0
  )} grams of Proteins.`;

  document.getElementById(
    "fatsText"
  ).innerText = `You need ${carbsMacro.toFixed(
    0
  ...
  document.getElementById(
    "carbsText"
  ).innerText = `You need ${fatsMacro.toFixed(
    0
